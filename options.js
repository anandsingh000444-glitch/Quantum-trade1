...
function prevTradingDay(d) {
  ...
  while(isHoliday(dt)) dt.setDate(dt.getDate()-1);
...
function getExpiry(sym) {
  ...
  const targetDay = EXPIRY_DAY[sym]||4;
  let diff = (targetDay - today + 7) % 7;
  if(diff===0 && hhmm>=930) diff=7;
  ist.setDate(ist.getDate()+diff);
  let expStr = ist.toISOString().slice(0,10);
  if(isHoliday(new Date(expStr))) expStr = prevTradingDay(new Date(expStr));
  return expStr;
}
...
export default async function handler(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers','Content-Type,x-upstox-token');
  if(req.method==='OPTIONS') return res.status(200).end();

  const sym = (req.query.sym||'NIFTY').toUpperCase();
  const instrKey = INSTR_MAP[sym];
  if(!instrKey) return res.status(400).json({error:'Unknown symbol: '+sym});

  // TOKEN PRIORITY: header > env UPSTOX_APP_TOKEN > env UPSTOX_TOKEN > hardcoded
  const headerToken = req.headers['x-upstox-token']||'';
  const UPSTOX_TOKEN = (headerToken && headerToken.length>50 ? headerToken : null)
    || process.env.UPSTOX_APP_TOKEN
  ...
  console.log('Token source:', headerToken.length>50?'HEADER':process.env.UPSTOX_APP_TOKEN?'ENV_APP':process.env.UPSTOX_TOKEN?'ENV_TOKEN':'HARDCODED');

  let expiry = req.query.expiry || getExpiry(sym);
  ...
  try {
    ...
    if(r.status===401) {
      ...
      return res.status(200).json({
        sym, expiry, sim:true, rows:[],
        error:'Token expired',
        message:'Upstox token expired. KY tab mein naaya token paste karein aur Save karein.'
      });
    }

    if(!r.ok) throw new Error('Upstox HTTP '+r.status);
    ...
    return res.status(200).json({sym,expiry,sim:true,rows:[],error:e.message});
  }
}
