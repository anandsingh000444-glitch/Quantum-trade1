// api/market.js - Yahoo Finance server-side (no CORS)
...
export default async function handler(req, res) {
  ...
  for(const url of urls){
    try{
      const r=await fetch(url,{headers:{'User-Agent':'Mozilla/5.0 (compatible)'},signal:AbortSignal.timeout(8000)});
      if(!r.ok)continue;
      ...
      if(m?.regularMarketPrice){
        ...
        return res.json({symbol:sym,price,change,changePct,high:+(m.regularMarketDayHigh||0),low:+(m.regularMarketDayLow||0),prevClose:prev,nseOpen,istTime,source:'yahoo',live:true,updatedAt:new Date().toISOString()});
      }
    }catch(e){console.log('Yahoo err:',e.message);}
  }

  return res.status(503).json({error:'Market data unavailable for '+sym,nseOpen,istTime,live:false});
}
