...
// Get all available expiries first, then pick nearest
async function getExpiries(instrKey) {
  ...
  if (!r.ok) throw new Error(`Upstox expiry ${r.status}`);
  const d = await r.json();
  // Extract unique expiry dates
  if (d.data && d.data.length > 0) {
    const expiries = [...new Set(d.data.map(x => x.expiry))].sort();
    return expiries;
  }
  throw new Error('No expiry data');
}

// Get nearest upcoming expiry (not today)
function getNearestExpiry(expiries) {
  const today = new Date().toISOString().slice(0, 10);
  const upcoming = expiries.filter(e => e >= today);
  return upcoming.length > 0 ? upcoming[0] : expiries[expiries.length - 1];
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!expiry) {
      try {
        const expiries = await getExpiries(instrKey);
        expiry = getNearestExpiry(expiries);
        console.log('Available expiries:', expiries.slice(0, 5));
        console.log('Selected expiry:', expiry);
      } catch(e) {
        // Fallback: calculate Thursday
        const ist = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
        const day = ist.getDay();
        const target = sym === 'BANKNIFTY' ? 3 : sym === 'SENSEX' ? 5 : 4;
        let diff = (target - day + 7) % 7;
        if (diff === 0) diff = 7;
        ist.setDate(ist.getDate() + diff);
        expiry = ist.toISOString().slice(0, 10);
      }
    }
